...
void vtkMINCWriter::Write()
{
    ...
    if( Transform )
        ...
        igstkLogMacro( DEBUG, "Write Transform :" << "\n" <<
        Transform->GetElement(0, 0) << ", " <<
        Transform->GetElement(0, 1) << ", " <<
        Transform->GetElement(0, 2) << ", " <<
        Transform->GetElement(1, 0) << ", " <<
        Transform->GetElement(1, 1) << ", " <<
        Transform->GetElement(1, 2) << ", " <<
        Transform->GetElement(2, 0) << ", " <<
        Transform->GetElement(2, 1) << ", " <<
        Transform->GetElement(2, 2) << ", " <<
        endl);
        igstkLogMacro( DEBUG, "Write idirCosine :" << "\n" <<
        idirCosine[0][0] << ", " <<
        idirCosine[0][1] << ", " <<
        idirCosine[0][2] << ", " <<
        idirCosine[1][0] << ", " <<
    ...
    {
        vtkReal tempStart[3] = { 0, 0, 0 };
        this->Input->GetOrigin( tempStart );
        vtkReal tempStep[3] = { 1, 1, 1 };
    ...
    dimVarId[0] = micreate_std_variable( mincFileId, MIzspace, NC_INT, 0, NULL );
    dimVarId[1] = micreate_std_variable( mincFileId, MIyspace, NC_INT, 0, NULL );
    dimVarId[2] = micreate_std_variable( mincFileId, MIxspace, NC_INT, 0, NULL );
    int index = 0;
    int i = 0;
    for( i = 0; i < 3; i++ )
    {
        index = 2 - i;
        miattputdbl( mincFileId, dimVarId[index], MIstep, istep[i] );
        miattputdbl( mincFileId, dimVarId[index], MIstart, istart[i] );
    ...
    case VTK_CHAR:
    {
	imageVarId = micreate_std_variable( mincFileId, MIimage, NC_BYTE, numberOfDimensions, dimId );
	miattputstr( mincFileId, imageVarId, MIsigntype, MI_UNSIGNED );
    }
    break;
    case VTK_SIGNED_CHAR:
    {
	imageVarId = micreate_std_variable( mincFileId, MIimage, NC_BYTE, numberOfDimensions, dimId );
	miattputstr( mincFileId, imageVarId, MIsigntype, MI_SIGNED );
    }
    break;
    ...
    case VTK_SHORT:
    {
	imageVarId = micreate_std_variable( mincFileId, MIimage, NC_SHORT, numberOfDimensions, dimId );
	miattputstr( mincFileId, imageVarId, MIsigntype, MI_SIGNED );
    }
    break;
    ...
    case VTK_LONG:
    {
	imageVarId = micreate_std_variable( mincFileId, MIimage, NC_LONG, numberOfDimensions, dimId );
	miattputstr( mincFileId, imageVarId, MIsigntype, MI_SIGNED );
    }
    break;
    ...
    case VTK_FLOAT:
    {
	imageVarId = micreate_std_variable( mincFileId, MIimage, NC_FLOAT, numberOfDimensions, dimId );
	miattputstr( mincFileId, imageVarId, MIsigntype, MI_SIGNED );
    }
    break;
    case VTK_DOUBLE:
    {
	imageVarId = micreate_std_variable( mincFileId, MIimage, NC_DOUBLE, numberOfDimensions, dimId );
	miattputstr( mincFileId, imageVarId, MIsigntype, MI_SIGNED );
    }
    break;
    default:
    ...
    int imageMaxVarId = micreate_std_variable( mincFileId, MIimagemax, NC_DOUBLE, 0, NULL);
    int imageMinVarId = micreate_std_variable( mincFileId, MIimagemin, NC_DOUBLE, 0, NULL);
	...
	mivarput( mincFileId, imageVarId, start, count, NC_BYTE, MI_UNSIGNED, ptrData );
    }
    break;
    case VTK_SIGNED_CHAR:
    {
	signed char * ptrData = (signed char*)Input->GetScalarPointer( 0, 0, 0 );
	mivarput( mincFileId, imageVarId, start, count, NC_BYTE, MI_SIGNED, ptrData );
    }
    break;
    case VTK_UNSIGNED_CHAR:
    {
	...
	mivarput( mincFileId, imageVarId, start, count, NC_SHORT, MI_SIGNED, ptrData );
    }
    break;
    case VTK_UNSIGNED_SHORT:
    {
	unsigned short * ptrData = (unsigned short*)Input->GetScalarPointer( 0, 0, 0 );
	mivarput( mincFileId, imageVarId, start, count, NC_SHORT, MI_UNSIGNED, ptrData );
    }
    break;
    case VTK_INT:
    {
	...
	mivarput( mincFileId, imageVarId, start, count, NC_LONG, MI_SIGNED, ptrData );
    }
    break;
    case VTK_UNSIGNED_LONG:
    {
	unsigned long * ptrData = (unsigned long*)Input->GetScalarPointer( 0, 0, 0 );
	mivarput( mincFileId, imageVarId, start, count, NC_LONG, MI_UNSIGNED, ptrData );
    }
    break;
    case VTK_FLOAT:
    {
	float * ptrData = (float*)Input->GetScalarPointer( 0, 0, 0 );
	mivarput( mincFileId, imageVarId, start, count, NC_FLOAT, MI_SIGNED, ptrData );
    }
    break;
    case VTK_DOUBLE:
    {
	double * ptrData = (double*)Input->GetScalarPointer( 0, 0, 0 );
	mivarput( mincFileId, imageVarId, start, count, NC_DOUBLE, MI_SIGNED, ptrData );
    }
    break;
    default:
	std::cerr<<"You really really screwed up"<<std::endl;
...
void vtkMINCWriter::WriteAdditionalMINCAttributes(int mincFileId)
{
    ...
    if (!Patient.empty())
    {
        patientVarId = micreate_std_variable( mincFileId, MIpatient, NC_INT, 0, NULL );
        PATIENT_ATTRIBUTES::iterator it = Patient.begin();
        PATIENT_ATTRIBUTES::iterator itEnd = Patient.end();
        for( ; it != itEnd; ++it )
        {
            miattputstr( mincFileId, patientVarId, (char*)(*it).first, (char*)(*it).second  );
        }
    }
    if (!Study.empty())
    {
        studentVarId = micreate_std_variable( mincFileId, MIstudy, NC_INT, 0, NULL );
        STUDY_ATTRIBUTES::iterator it = Study.begin();
        STUDY_ATTRIBUTES::iterator itEnd = Study.end();
        for( ; it != itEnd; ++it )
        {
            miattputstr( mincFileId, studentVarId, (char*)(*it).first, (char*)(*it).second  );
        }
    }
    if (!Acquisition.empty())
    {
        acquisitionVarId = micreate_std_variable( mincFileId, MIacquisition, NC_INT, 0, NULL );
        ACQUISITION_ATTRIBUTES::iterator it = Acquisition.begin();
        ACQUISITION_ATTRIBUTES::iterator itEnd = Acquisition.end();
        for( ; it != itEnd; ++it )
        {
            miattputstr( mincFileId, acquisitionVarId, (char*)(*it).first, (char*)(*it).second  );
        }
    }
    if (Comment)
        miattputstr( mincFileId, -1, MIcomments, Comment );
