/*=========================================================================
  ...
  Module:    $RCSfile: vtkNShapeCalibrationWidget.cxx,v $
  Language:  C++
...
vtkCxxRevisionMacro(vtkNShapeCalibrationWidget, "$Revision: 1.5 $");
...
void vtkNShapeCalibrationWidget::PrintSelf(ostream& os, vtkIndent indent)
{
    ...
    vtkReal *pt1 = this->LineSource->GetPoint1();
    vtkReal *pt2 = this->LineSource->GetPoint2();
...
void vtkNShapeCalibrationWidget::OnMouseMove()
{
    ...
    {
        vtkReal factor = 1.0 + (vtkReal)(lastY - Y) / (vtkReal)100.0;
...
void vtkNShapeCalibrationWidget::Scale(double *p1, double *p2, int vtkNotUsed(X), int Y)
{
    ...
    vtkReal sf = vtkMath::Norm(v) / sqrt(vtkMath::Distance2BetweenPoints(pt1,pt2));
...
void vtkNShapeCalibrationWidget::PlaceWidget(vtkReal bds[6])
{
    int i;
...
void vtkNShapeCalibrationWidget::SetPoint1(vtkReal x, vtkReal y, vtkReal z) 
{
    vtkReal xyz[3];
...
void vtkNShapeCalibrationWidget::SetPoint2(vtkReal x, vtkReal y, vtkReal z) 
{
    vtkReal xyz[3];
    xyz[0] = x; xyz[1] = y; xyz[2] = z;
...
void vtkNShapeCalibrationWidget::SetMiddlePoint( vtkReal x, vtkReal y, vtkReal z )
{
    vtkReal * pt1 = this->GetPoint1();
    vtkReal * pt2 = this->GetPoint2();
    vtkReal v[3];
    v[0] = pt2[0] - pt1[0];
    v[1] = pt2[1] - pt1[1];
    v[2] = pt2[2] - pt1[2];
...
void vtkNShapeCalibrationWidget::GetMiddlePoint( vtkReal xyz[3] )
{
    vtkReal * pt1 = this->GetPoint1();
    vtkReal * pt2 = this->GetPoint2();
...
vtkReal vtkNShapeCalibrationWidget::GetMiddlePoint()
{
    return this->MiddleHandlePosition;
}


void  vtkNShapeCalibrationWidget::SetMiddlePoint( vtkReal ratio )
{
    this->MiddleHandlePosition = ratio;
}


void vtkNShapeCalibrationWidget::SetHandlesSize( vtkReal size )
{
...
void vtkNShapeCalibrationWidget::ScaleHandles( vtkReal f )
{
    vtkReal outerRadius = this->HandleGeometry->GetOuterRadius();
    outerRadius *= f;
...
void vtkNShapeCalibrationWidget::SetLinePosition( vtkReal x, vtkReal y, vtkReal z ) 
{
    vtkReal p1[3], p2[3], v[3];
...
void vtkNShapeCalibrationWidget::ClampPosition(vtkReal x[3])
{
    for (int i=0; i<3; i++)
    {
...
int vtkNShapeCalibrationWidget::InBounds(vtkReal x[3])
{
    for (int i=0; i<3; i++)
    {
        if ( x[i] < this->InitialBounds[2*i] ||
